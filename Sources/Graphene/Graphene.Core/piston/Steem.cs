...
using Graphene.Core.Extensions;
...
namespace Graphene.Core
{
    ...
    public class Steem
    {
        public SteemNodeRPC RPC { get; set; }
        ...
        /// <summary>
        /// to prevent connecting to network (defaults to ``false``)
        /// </summary>
        ...
        /// 
        /// </summary>
        /// <param name="node">Node to connect to *(optional)*</param>
        /// <param name="rpcuser">RPC user *(optional)*</param>
        /// <param name="rpcpassword">RPC password *(optional)*</param>
        /// <param name="debug">Enable Debugging *(optional)*</param>
        /// <param name="kwargs"></param>
        public Steem(string node = "", string rpcuser = "", string rpcpassword = "", bool debug = false, Dictionary<string, object> kwargs = null)
        {
            if (kwargs == null)
                kwargs = new Dictionary<string, object>();

            //# More specific set of APIs to register to
            if (!kwargs.ContainsKey("apis"))
                kwargs.Add("apis", "[\"database\",\"network_broadcast\",\"account_by_key\",\"follow\",]");

            RPC = null;
            Debug = debug;
            Offline = kwargs.Get("offline", false);
            NoBroadcast = kwargs.Get("nobroadcast", false);
            UnSigned = kwargs.Get("unsigned", false);
            Expiration = kwargs.Get("expiration", 30);
            if (!Offline)
                Connect(node, rpcuser, rpcpassword, kwargs);
            Wallet = new Wallet(RPC, null, kwargs);
        }
        ...
        /// <param name="kwargs"></param>
        internal void Connect(string node = "", string rpcuser = "", string rpcpassword = "", Dictionary<string, object> kwargs = null)
        {
            if (string.IsNullOrEmpty(node))
            {
                if (Config.ContainsKey("node"))
                    ...
                    throw new NullReferenceException("A Steem node needs to be provided!");
            }

            if (string.IsNullOrEmpty(rpcuser) && Config.ContainsKey("rpcuser"))
                rpcuser = (string)Config["rpcuser"];

            if (string.IsNullOrEmpty(rpcpassword) && Config.ContainsKey("rpcpassword"))
                rpcpassword = (string)Config["rpcpassword"];

            RPC = new SteemNodeRPC(node, rpcuser, rpcpassword, kwargs);
        }
